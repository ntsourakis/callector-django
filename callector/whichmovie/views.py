...
import uuid
import sys
import whichmovie.python.lite_call_runtime_top as call
import whichmovie.python.lite_call_output_manager as output_manager
import os 

States = {};

def WhichMoviePageView(request):
    return HttpResponse('Hello, WhichMovie!')

# Class for initialization
class WhichMovieInitView(APIView):
    ...
    def get(self, request): 
        ...
        ( State, InitAct, DontUnderstandAct ) = WhichMovieInitView.init(userId)
...
class WhichMovieMessageView(APIView):
    permission_classes = (IsAuthenticated,)
    ...
    def post(self, request): 
        ...
        highScore = WhichMovieMessageView.getUserHighScore(userId)
        ...
        if 'presented_score' in action:
            ...
            if scoreList != None:
                ...
                position.append(WhichMovieMessageView.getScorePosition(scoreList, action['presented_score'][0]))
        ...
        if 'end' in action and action['end'] == 'yes':
            WhichMovieMessageView.deleteState(userId)
