...
import uuid
import sys
import lettergame.python.lite_call_runtime_top as call
import lettergame.python.lite_call_output_manager as output_manager
import os 

States = {};

def LetterGamePageView(request):
    return HttpResponse('Hello, LetterGame!')

# Class for initialization
class LetterGameInitView(APIView):
    ...
    def get(self, request): 
        ...
        ( State, InitAct, DontUnderstandAct ) = LetterGameInitView.init(userId)
...
class LetterGameMessageView(APIView):
    permission_classes = (IsAuthenticated,)
    ...
    def post(self, request): 
        ...
        highScore = LetterGameMessageView.getUserHighScore(userId)
		
        # Ensure that we are at the end of the lesson
        if 'presented_score' in action:
            LetterGameMessageView.saveScore(action['presented_score'][0])
        
            # Store only the best score
            if highScore < action['presented_score'][0]:
                LetterGameMessageView.saveUserHighScore(userId, action['presented_score'][0])
            
            scoreList = LetterGameMessageView.getAllUserScores()
            print(type(scoreList))
			
            if scoreList != None:
                position = []
                #Compare the new high score against all high scores 
                position.append(LetterGameMessageView.getScorePosition(scoreList, action['presented_score'][0]))
        ...
        if 'end' in action and action['end'] == 'yes':
            LetterGameMessageView.deleteState(userId)
