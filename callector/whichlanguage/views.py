...
import uuid
import sys
import whichlanguage.python.lite_call_runtime_top as call
import whichlanguage.python.lite_call_output_manager as output_manager
import os 

States = {};

def WhichLanguagePageView(request):
    return HttpResponse('Hello, WhichLanguage!')

# Class for initialization
class WhichLanguageInitView(APIView):
    ...
    def get(self, request): 
        ...
        ( State, InitAct, DontUnderstandAct ) = WhichLanguageInitView.init(userId)
...
class WhichLanguageMessageView(APIView):
    permission_classes = (IsAuthenticated,)
    ...
    def post(self, request): 
        ...
        highScore = WhichLanguageMessageView.getUserHighScore(userId)
		
        # Ensure that we are at the end of the lesson
        if 'presented_score' in action:
            WhichLanguageMessageView.saveScore(action['presented_score'][0])
        
            # Store only the best score
            if highScore < action['presented_score'][0]:
                WhichLanguageMessageView.saveUserHighScore(userId, action['presented_score'][0])
            
            scoreList = WhichLanguageMessageView.getAllUserScores()
            print(type(scoreList))
			
            if scoreList != None:
                position = []
                #Compare the new high score against all high scores 
                position.append(WhichLanguageMessageView.getScorePosition(scoreList, action['presented_score'][0]))
        ...
        if 'end' in action and action['end'] == 'yes':
            WhichLanguageMessageView.deleteState(userId)
