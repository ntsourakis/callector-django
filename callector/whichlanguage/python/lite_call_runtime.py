...
import whichlanguage.python.lite_call_output_manager as output_manager
import whichlanguage.python.lite_call_tables as tables
import whichlanguage.python.lite_call_scores as scores
import whichlanguage.python.lite_call_logging as log

import whichlanguage.python.lite_grammar_match as lgm
import whichlanguage.python.lite_grammar_match_tables as lgm_tables
import whichlanguage.python.lite_grammar_match_regularise as lgm_regularise
...
def get_initial_state(Namespace, Domain, L1):
    ...
    output_manager.prepend_invocation_name_and_introduction_to_action_text(InitialAction, State)
...
def continue_from_saved_state(State):
    AbstractAction = { 'type':'prompt',
                       'response_to':'set_lesson',
                       'text':State['prompt'],
                       'visual_text':State['visual_prompt'],
                       'text_to_repeat':State['prompt'],
                       'visual_text_to_repeat':State['visual_prompt'],
                       'lesson':name_for_lesson(State['lesson'], State),
...
def init_state(TableFile, RobustMatchFile):
    ( Namespace, Domain, L1 ) = load_tables(TableFile, RobustMatchFile)
    return get_initial_state(Namespace, Domain, L1)

def init_state_basic(TableFile, RobustMatchFile):
    ( Namespace, Domain, L1 ) = load_tables(TableFile, RobustMatchFile)
    return get_initial_state_basic(Namespace, Domain, L1)

def continue_state(State, userId):
    if can_continue_state(State, userId):
        print('Trying to continue state (lite_call_runtime)')
        AbstractAction = { 'type':'prompt',
                           'response_to':'set_lesson',
                           'text':State['prompt'],
                           'visual_text':State['visual_prompt'],
        ...
        print('Error: illegal attempt to try and continue from state')

def can_continue_state(State, userId):
    if userId_matches_in_state(State, userId):
        if not 'prompt' in State:
            print('prompt not found in State')
            return False
        elif not 'lesson' in State:
            print('lesson not found in State')
            return False
        elif not 'language' in State:
            print('language not found in State')
            return False
        elif not 'l2' in State:
            print('l2 not found in State')
            return False
        else:
            print('prompt, lesson, language and l2 all found in State. Can continue.')
            return True
    else:
        return False
        
def userId_matches_in_state(State, userId):
    print('New userId = ' + userId)
    if 'userId' in State:
        oldUserId = State['userId']
        print('Old userId = ' + oldUserId)
        if userId == oldUserId:
            print('userIds are the same')
            return True
        else:
            print('userIds are different')
            return False
    else:
        print('No old userId')
        return False
...
def default_help_action(State):
    ...
    Action = { 'text': DefaultHelpText,
               'visual_text': DefaultHelpText
               }
    return Action
...
def string_and_state_to_action_main(String0, State):
    ...
    if MessageInterpretation:
        ...
        AbstractAction1 = process_call_message(['match', String0, String], State)
...
def do_auto_next_if_necessary(Action1, State, L1, L2):
    if 'auto_next' in Action1 and Action1['auto_next'] == 'auto_next':
        ...
        Action2['visual_text'] = Action1['visual_text'] + ' ' + Action2['visual_text']
        if 'presented_score' in Action1:
            Action2['presented_score'] = Action1['presented_score']
...
def well_formed_message(Message):
    if not isinstance(Message, (list, tuple)):
        ...
        return True
    elif Message[0] == 'match' and ( len(Message) == 2 or len(Message) == 3 ):
        return True
...
def process_call_message(Message, State):
    if not well_formed_message(Message):
        ...
        return match(Message[1], Message[1], State)
    elif Message[0] == 'match' and len(Message) == 3:
        return match(Message[1], Message[2], State)
    elif Message[0] == 'exit':
...
def set_next_lesson(State):
    NextLesson = get_next_lesson(State)
    if NextLesson:
        return set_lesson(State, NextLesson)
    else:
        return { 'response_to':'set_lesson',
                 'error':'no_more_lessons'}
...
def set_lesson(State, Lesson):
    if valid_lesson_in_state(State, Lesson):
        ...
        AllLessons = get_all_current_lessons(State)
        # If we've just started the first lesson, reset the score
        if AllLessons and Lesson == AllLessons[0]:
            State['score'] = 0
            State['max_possible_score'] = 0
        # If we've just started the last lesson (which will also be the first lesson if there is only one lesson)
        # then mark that in the state
        if AllLessons and Lesson == AllLessons[-1]:
            State['on_last_lesson'] = True
        else:
            State['on_last_lesson'] = False
...
def set_next_prompt(State):
    if 'forwardList' in State and len(State['forwardList']) > 0 :
        ...
        if 'on_last_lesson' in State:
            on_last_lesson = State['on_last_lesson']
        else:
            on_last_lesson = False
        return { 'response_to':'next', 'error':'no_more_prompts_in_lesson',
                 'on_last_lesson':on_last_lesson,
...
def set_next_prompt_using_forward_list(State):
    ...
    Prompt = { 'type':'prompt', 'response_to':'next', 'text':State['prompt'], 'visual_text':State['visual_prompt'] }
    return Prompt
...
def forward_list_for_all_prompts(State):
    ...
    return forward_list_for_all_prompts1(Feedback, OrderedGroups, AllPrompts, Language, State)

def forward_list_for_all_prompts1(Feedback, OrderedGroups, AllPrompts, Language, State):
    if len(OrderedGroups) == 0:
        return []
    else:
        F = OrderedGroups[0]
        R = OrderedGroups[1:]
        ForwardListF = forward_list_for_prompts_in_group(Feedback, AllPrompts[F], Language, State)
        ForwardListR = forward_list_for_all_prompts1(Feedback, R, AllPrompts, Language, State)
        return ForwardListF + ForwardListR

def forward_list_for_prompts_in_group(Feedback, PromptRecordsForGroup, Language, State):
    if Feedback == 'alexa_minidialogue':
        return [ random_member(prompt_records_to_forward_list(PromptRecordsForGroup, Language, State)) ]
    else:
...
def prompt_record_to_forward_list(PromptRecord, Language, State):
    ...
    if not null_multimedia(Multimedia):
        ...
        Prompt = PromptText
        VisualPrompt = PromptText
    return { 'prompt':Prompt, 'visual_prompt':VisualPrompt, 'canonical':Canonical, 'example':Example }
...
def get_help(State):
    ...
    if HelpTextAndNItems:
        ...
        Help = { 'type':'prompt', 'response_to':'help', 'text':HelpText, 'n_help_items':NHelpItems,
                 'prompt':State['prompt'], 'visual_prompt':State['visual_prompt'] }
...
def wait(State):
    return { 'type':'wait', 'response_to':'wait', 'text':State['prompt'], 'visual_text':State['visual_prompt'] }
...
def get_max_tries_for_state(State):
    ...
    if CourseInfo and 'maxtries' in CourseInfo:
...
def get_help_strategy_for_state(State):
    CourseInfo = course_info_for_state(State)
    if CourseInfo and 'helpstrategy' in CourseInfo:
...
def get_last_system_utterance(State):
    if 'text_to_repeat' in State and 'visual_text_to_repeat' in State:
        Help = { 'type':'repeat', 'response_to':'repeat',
                 'text':State['text_to_repeat'], 'visual_text':State['visual_text_to_repeat'] }
        return Help
    else:
...
def match(RawString, String, State):
    Key0 = tuple([State['namespace']])
    ...
    return merge_dicts( MatchStructure,
                        { 'type':'match_response', 'response_to':'match',
                          'match_string':String, 'raw_match_string':RawString,
                          'prompt':State['prompt'], 'visual_prompt':State['visual_prompt'],
...
def get_next_lesson(State):
    Lessons = get_all_current_lessons(State)
    if Lessons and 'lesson' in State:
        return next_lesson_from_list(Lessons, State['lesson'])
    else:
        return False

def get_all_current_lessons(State):
...
def get_available_lessons(State):
    if ( not 'namespace' in State ) or ( not 'domain' in State ):
        Lessons = []
    elif not tuple([State['namespace']]) in tables.stored_lesson:
        Lessons = []
    else:
...
def course_info_for_namespace_and_domain(Namespace, Domain):
    ...
    if not ( Key0 in tables.course_info ):
        print(f'*** Warning: no course info available for {Namespace}')
        return False
    CoursesForNamespace = tables.course_info[Key0]
    Key = tuple([Domain, Namespace])
    if not ( Key in CoursesForNamespace ):
        print(f'*** Warning: no course info available for {Domain}')
        return False
...
def update_state_from_forward_or_back_list_item(State, Item):
    ( Prompt, VisualPrompt, Example, Canonical ) = ( Item['prompt'], Item['visual_prompt'], Item['example'], Item['canonical'] )
    State['prompt'] = Prompt
...
def forward_or_back_list_item_from_state(State):
    if 'prompt' in State and 'visual_prompt' in State and 'example' in State and 'canonical' in State:
        Item = {}
        Item['prompt'] = State['prompt'] 
        Item['visual_prompt'] = State['visual_prompt']
...
def current_prompt_for_state(State):
    Prompt = { 'text':State['prompt'], 'visual_text':State['visual_prompt'] }
    return Prompt

# Robust matching

def robust_process_string_for_call(String, State):
    MatchResults = lgm.match_string(String, 1, State['namespace'], State['domain'], State['l2'])
    if len(MatchResults) > 0 and plausible_match(MatchResults[0]):
        ...
        # Even if we decide the robust matching results aren't plausible, still regularise
        return lgm_regularise.regularise_string(String, State['namespace'], State['domain'], State['l2'])
