...
import whichlanguage.python.lite_grammar_match_parameters as parameters
import whichlanguage.python.lite_grammar_match_regularise as regularise
import whichlanguage.python.wagnerfischer as w

# TOP-LEVEL CALLS

# Match a string with a context
def match_string(InputString, NReturned, Namespace, Domain, Language):
    Context = ''
    return match_string_with_context(InputString, Context, NReturned, Namespace, Domain, Language)

# Match a string
def match_string_with_context(InputString, Context, NReturned, Namespace, Domain, Language):
    NShortlist = 13
    InputStringAndContext = (InputString, Context)
    return match_string1(InputStringAndContext, NShortlist, NReturned, Namespace, Domain, Language)

# Match a string, specifying the length of the shortlist
def match_string1(InputStringAndContext, NShortlist, NReturned, Namespace, Domain, Language):
    Words = regularise.convert_input_and_context_to_words_list(InputStringAndContext, Namespace, Domain, Language)
